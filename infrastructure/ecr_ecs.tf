# =============================================================================
# ECR and ECS Fargate Configuration
# =============================================================================
# ECRリポジトリとECS Fargateクラスター/サービスを定義します。

# -----------------------------------------------------------------------------
# ECR Repository
# -----------------------------------------------------------------------------
# バックエンドAPIのDockerイメージを保存
...
# ECSタスク実行ロール（ECR、CloudWatch Logsへのアクセス）
...
# ECSタスクロール（アプリケーションが使用するAWSリソースへのアクセス）
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = aws_dynamodb_table.device_tokens.arn
      }
    ]
  })
}
...
# HTTPリスナー（開発環境用、本番ではHTTPSにリダイレクト）
