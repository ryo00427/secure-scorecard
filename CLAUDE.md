...
- カレンダー: 完全に未実装
- 作物追加: 画像アップロードが未実装
- 多くの画面で `Alert('実装中です')` のプレースホルダーが残存

**原因**:

- タスク定義が曖昧だった（「画面実装」だけでデザイン参照が明記されていない）
- 完了基準が不明確（「コンパイルが通る」≠「デザイン通り」）
- デザインファイルを確認せずに「動く画面」だけ作成した
- プレースホルダー（Alert）を入れたまま完了とした

**対策**: UI実装時は必ずデザイン照合を行う

```
UI実装開始前チェックリスト:
□ デザインファイルを確認する (design/ フォルダ)
□ 実装する画面のデザインを読み込む
□ 必要なコンポーネント・機能を洗い出す
□ タスクに「デザインファイル: xxx」を明記する

UI実装中チェックリスト:
□ デザインと実装を並べて比較しながら作業
□ プレースホルダー（Alert、TODO、実装中です）を使わない
□ 機能が未実装なら画面自体を作らない or 明確に「未実装」とタスクに残す

UI実装完了チェックリスト:
□ デザインファイルと実装画面をピクセル単位で比較
□ すべてのUI要素が存在するか確認:
  - アイコン、画像、色、フォント
  - レイアウト、余白、配置
  - インタラクション（タップ、スワイプ）
□ プレースホルダーが残っていないか検索
  grep -r "実装中" apps/mobile/
  grep -r "TODO" apps/mobile/
  grep -r "Alert\(" apps/mobile/
□ 完了としてよいか自問:
  「このUIをユーザーに見せて恥ずかしくないか？」
```
...
- [ ] Task 12.5: モバイルUI画面実装
  ...
  - 完了基準:
    ...
    □ 作物画像表示機能
    □ 注目タスクカード
    □ デザインとの差分なし（スクリーンショット比較済み）
```

**重要**: 「動く画面」≠「完成した画面」。デザイン通りになるまで完了としない。
...
grep -r "FIXME" apps/
grep -r "実装中" apps/
grep -r "Alert\(" apps/mobile/  # Alertは本番では使わない
```

**重要**: 「後で直す」は「直さない」と同義。今完成させるか、タスクを分割する。

### 13. フロントエンドのAPI型定義はバックエンドの実装を確認してから書く（最重要）

**失敗例**: フロントエンドで `{ crops: Crop[] }` を期待する型を定義したが、バックエンドは配列を直接返していた
...
// HomeScreen.tsx
const growingCrops = crops?.crops?.filter((c) => c.status === 'growing');
// → crops?.crops が undefined で何も表示されない
```

```go
// 実際のバックエンド (crop.go)
func (h *Handler) GetCrops(c echo.Context) error {
    crops, _ := h.service.GetUserCrops(ctx, userID)
    return c.JSON(http.StatusOK, crops)  // 配列を直接返す！
}
```

**原因**:

- バックエンドのコードを読まずにフロントエンドの型を「推測」で書いた
- `{ data: [...] }` のようなラッパーがあると思い込んだ
- 型エラーが出ないため、実行時まで問題に気づかない

**対策**: API型定義前にバックエンドを必ず確認する

```
API型定義のチェックリスト:
□ バックエンドのハンドラー関数を読む
  - handler/*.go の該当エンドポイント
  - return c.JSON(http.StatusOK, ???) の引数を確認
□ 実際にcurlで叩いてレスポンスを確認
...
```bash
# 1. まずバックエンドのレスポンスを確認
curl -s http://localhost:8080/api/v1/crops | jq
# 出力: [{...}, {...}]  ← 配列が直接返る

# 2. バックエンドのコードを確認
# crop.go:133: return c.JSON(http.StatusOK, crops)
...
1. **OpenAPI/Swagger** でAPI仕様を一元管理する
2. **openapi-typescript** 等で型を自動生成する
3. **統合テスト** でフロントエンド↔バックエンドの型を検証する

**重要**: フロントエンドの型は「期待」ではなく「事実」に基づいて定義する。推測で書かない。
